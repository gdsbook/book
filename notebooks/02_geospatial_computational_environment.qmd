---
title: Computational Tools for Geographic Data Science
jupyter: gds
---
...
used in a variety of contexts with slightly different meanings [@cruwell2019seven], one statement of the intution behind Open Science is that the
...
similar topics albeit in a much more opaque way [@Nielsen_2020]. Scientists
...
```{python}
#| caption: Galileo's drawings of Jupyter and the Medician stars, showing the power of diagrams inside of scientific texts.
#| echo: false
url = (
...
crisis [@Ioannidis_2007]. This "crisis" arises because the analyses 
...
sciences, this was probably easier: quantitative data was scarce and much of the analysis relied either on math to minimize the amount of computation required [@efron2016computer] or small datasets
...
for action [@Rey_2009]. On the back of these debates, the term
"reproducibility" is also gaining traction. Again, this is a rather general
term but in one popular definition [@bollen2015social], it suggests that
...
have several advantages for geographic data science work [@boeingAB2020].
...
to write notebooks in efficient ways is also emerging (e.g., [@Rule_2019]), so this too represents an evolving set of practices which we document incompletely in the following sections.
...
data science are starting to be explored [@Cook_2017].
...
Our choice of notebook system is Jupyter [@Kluyver2016jupyter]. A Jupyter notebook is a plain
...
[@ghfmd] that is adopted by Jupyter notebook.
...
```{python}
#| caption: Embedding rich media in a notebook.
...
```{python}
#| caption: This book's logo, built from Stamen Toner map tiles and from code.
...
```{python}
#| caption: 'An annotated view of the JupyterLab interface '
#| echo: false
...
to courses in economics by Nobel Prize-winning professors[^quantecon], Python is a fundamental component of "consensus" data science [@donoho201750].
...
options for Data Science [@te_python]. Python is widely used for data 
...
```{python}
#| caption: The authentification screen for Jupyter notebooks
#| echo: false
