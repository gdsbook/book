...
To illustrate these considerations, we will examine regional income data for 32 Mexican states [@Rey_2010] in this chapter. The variable we focus on is per capita gross domestic product
for 1940 (`PCGDP1940`):
...
interval width ($w$) and the number of bins in `seaborn` is based on the Freedman-Diaconis rule [@freedman1981histogram]:
...
The head tail algorithm [@Jiang_2013] is based on a recursive partitioning of the data using splits around
...
Originally proposed by [@Jenks_1971], the Jenks-Caspall classification algorithm
aims to minimize
...
building method [@Duque_2011] to the case of map classification. It is similar in spirit to
...
[@Brewer1997mapping]. Generally, a distinction is drawn between three
...
- [@slocum2014thematic]. Thematic Cartography and Geovisualization. Pearson.
